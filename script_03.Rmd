...
```{r Scene 2.1}
# - Identify the problem at hand: analyzing class enrollment numbers 
# - Narrow down data to fit problem: eliminate all columns besides id, title,
# name, department, u_grad
# - precisely define estimand: (enrollment_new - enrollment_old)
```
...
```{r Scene 2.2}
# - realize that real-world data is not there for us to  examine
# - understand that correlation does not always equal causation
# - be aware of internal and external validity
```
...
```{r Scene 3}
sep_dif <-
  full_join(sep_old, sep_new, by = c("id"), keep = TRUE, suffix = c("_old", "_new")) %>%
  mutate(enrollment_diff = u_grad_new - u_grad_old) %>%
  select(id_new, title_new, department_new, u_grad_old, u_grad_new, enrollment_diff) %>%
